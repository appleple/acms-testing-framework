...
final class Bootstrap
{
    ...
    private static function defineApplicationConstants(string $bodyRoot): void
    {
        ...
        $constants = [
            ...
            'ROOT_DIR' => '',
            'SETUP_DIR' => '',
            // setup 専用 Twig（@setup 名前空間）が SETUP_PATH . 'templates' を解決するため、本体ツリーの
            // setup/ を指す既定値を与える（本番では setup/setup-application.php が実行中ディレクトリで定義する）。
            'SETUP_PATH' => $bodyRoot . '/setup/',
        ];
        foreach ($constants as $name => $value) {
            self::defineConstant($name, $value);
        }
