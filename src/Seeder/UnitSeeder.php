...
class UnitSeeder extends Seeder
{
    ...
    private static function insertColumnInternal(
        ...
        $id = (string) self::getOrFake($data, 'column_id', self::generateUuid());
        ...
        // カスタムユニットの custom_xxx のようにサブタイプを持つユニットのため、$data からの上書きを許す
        $sql->addInsert('column_type', self::getOrFake($data, 'column_type', $type));
    ...
    /**
     * column_rev テーブルにカスタムユニットを作成
     *
     * フィールドは作成しない。リビジョンのフィールドが必要な場合は FieldSeeder::seedRev() を使う。
     *
     * @param int $entryId
     * @param int $blogId
     * @param int $rvid リビジョンID
     ...
     * @return string 生成した column_id
     */
    public static function seedCustomRev(int $entryId, int $blogId, int $rvid, array $data = []): string
    {
        return self::insertColumnRev($entryId, $blogId, $rvid, Custom::getUnitType(), $data);
    }
