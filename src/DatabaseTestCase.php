...
abstract class DatabaseTestCase extends TestCase
{
    ...
    protected function tearDown(): void
    {
        ...
        if (Database::connection()->inTransaction()) {
            ...
            // DDL は暗黙コミットを起こしてトランザクションを閉じるため、上の分岐に入らない。
            // その後に実行された DML が新しいトランザクションを開いている可能性があるので、
            // autocommit を戻す前に明示的に破棄する（SET autocommit=1 は未コミットの変更を
            // ロールバックではなくコミットしてしまうため、順序が重要）。
            Database::query('ROLLBACK', 'exec');
        }

        // 自動コミットを元に戻す。
        // ロールバックの分岐内に置くと、DDL で暗黙コミットが起きたテストの後に autocommit=0 が
        // 次のテストへ漏れる。次のテストの setUp は beginTransaction() より前に実テーブルへ触れる
        // ため、その時点で InnoDB が新しいトランザクションを開き、beginTransaction() が
        // 「There is already an active transaction」で失敗して静かにスキップされてしまう。
        if ($this->autoCommit) {
