 ...
 * constants for setup
 *
 * setup.php 冒頭（現在は setup/setup-application.php のトップレベル）で実行時に define される
 * setup 専用のパス定数。autoload 化した Acms\Services\Setup\* クラス（SetupCommon / Detector 等）が
 * これらを参照するため、存在解決用にここへ置く（値は dynamicConstantNames で動的扱い）。
 */
define('ROOT_DIR', '');
define('SETUP_DIR', '');
define('SETUP_PATH', '');
...
define('PATH_CONFIG', '');
define('PATH_LICENSE', '');
define('PATH_HTACCESS', '');
define('PATH_ENV', '');
define('PATH_ARCHIVES', '');
define('PATH_MEDIA', '');
define('PATH_STORAGE', '');
define('PATH_THEMES', '');
define('PATH_CACHE', '');
define('REGEX_VALID_DOMAIN', '');
